...
static struct
{
    ...
    adv_beacon_handler_t	adv_handler;
} m_beacon;
...
nrf_radio_request_t * m_configure_next_event(void)
{
    ...
    m_beacon.timeslot_request.params.normal.hfclk       = NRF_RADIO_HFCLK_CFG_XTAL_GUARANTEED; // CF: Not sure which to use. This is the closest one though I think.
...
static uint8_t * m_get_adv_packet(void)
{
    ...
    // Constructing header
    adv_pdu[offset]    = 1 << 1; // BLE_GAP_ADV_TYPE_ADV_NONCONN_IND; // Advertisement type ADV_NONCONN_IND
    ...
    adv_pdu[offset++] =  BLE_GAP_ADV_FLAG_BR_EDR_NOT_SUPPORTED;
...
void app_beacon_init(ble_beacon_init_t * p_init)
{
    ...
    m_beacon.adv_handler  = p_init->adv_handler;
}
...
void app_beacon_start(void)
{
    ...
    if ((err_code != NRF_SUCCESS) && (m_beacon.error_handler != NULL))
    {
    	...
    	if (m_beacon.adv_handler != NULL) {
            m_beacon.adv_handler(true);
        }
    }
}
